...
function ProductInfo({ productId }) {
  ...
  if (isError || !product) {
    return (
      <div className="flex items-center gap-2 p-3 bg-slate-50 rounded-xl text-slate-400">
  ...
  return (
    <div className="flex items-center gap-3 p-3 bg-slate-50 rounded-xl">
      <img
        ...
        alt={product.title}
        className="w-14 h-14 rounded-lg object-cover flex-shrink-0 border border-slate-200 bg-white"
        onError={(e) => {
      ...
      <div className="min-w-0">
        ...
        <p className="text-sm font-bold text-slate-700 mt-0.5">${product.price}</p>
      </div>
    </div>
  );
}
...
function TicketDetailSkeleton() {
  return (
    <div className="p-4 sm:p-8 max-w-5xl animate-pulse">
      <div className="h-5 bg-slate-200 rounded w-36 mb-8" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 h-[480px] bg-slate-200 rounded-xl" />
      </div>
    </div>
  );
}
  ...
  return (
    <div className="p-4 sm:p-8 max-w-5xl">
      ...
      <div className="flex items-start justify-between mb-7 gap-4">
        ...
        {ticket.status === 'open' && (
          <button
            onClick={handleClose}
            disabled={isClosing}
            className="flex items-center gap-2 px-4 py-2 border border-red-200 text-red-600 rounded-xl hover:bg-red-50 disabled:opacity-50 transition-colors text-sm font-medium flex-shrink-0"
          >
            <XCircle size={15} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-4">
          <div className="bg-white rounded-xl border border-slate-200 p-4 space-y-3">
            ...
            <div className="flex items-center gap-2 text-sm">
              ...
              <span className="text-slate-500 truncate">{ticket.customer_email}</span>
            </div>
            <div className="flex items-center gap-2 text-sm">
        ...
        {/* Conversation panel */}
        <div className="lg:col-span-2 bg-white rounded-xl border border-slate-200 flex flex-col min-h-[480px]">
